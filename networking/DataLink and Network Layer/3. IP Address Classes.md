IP addresses can be split into two sections, the network ID and the host ID. IBM owns all IP addresses that have a nine as the value of the first octet in an IP address. 
